...
RUTA_RULES=$PWD"/RutaRules/MMSE/MMSE.ruta"
OUT1=$OUTDIR/$NAME"_"$TIMESTAMP
LOG_FILE=$LOGDIR"/"$RULES"_"$TIMESTAMP".log"
...
echo IN File count for $RULES: $FILECOUNT
...
>> $LOG_FILE 2>&1
duration8=$SECONDS
echo $RULES finished in $(($duration8 / 60)):$(($duration8 % 60))
echo $RULES documents/s: $(($FILECOUNT / $duration8))
SECONDS=0
...
TEMPLATE="mmse"
IN1=$OUT1
OUT2=$OUTDIR/$NAME"_"$TEMPLATE"_"$TIMESTAMP
